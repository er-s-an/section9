...
公开 `main` 的可复跑路径是固定小智示例应用的故障实验与 Pair 对照。`support-agent` 外部候选属于后续本地开发分支，其代码未随本次文档上传发布到 `main`；两者不能拼成一次没有实际发生的完整事故。控制端、有限工具、事件库和观测栈在本机；推理依赖远程 EvoMap API。公开版本的结果与失败见[交付记录](DELIVERY.md)。
...
本地开发分支已探索 `support-agent` 的接入、订单/退款只读真值探针、独立回归验收、本地测试发布和短观察。本次仅上传展示规划，不上传那部分代码或对应分支的验收产物。公开 `main` 当前不能按此流程复跑；它也不是最终主故事中的跨视野复合事故。
