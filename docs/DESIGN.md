# Section9 前端设计：当前实现与目标展位

目标展示让观众先看见业务故障与恢复，再理解蜂群协作过程，最后检查对照数据和原始证据。推荐页面层级是“演示首页 → 事故与协作 → 方案与验收 → 对照实验”；事故页中的办公室用于定位成员和任务，证据卡与因果链负责解释结论。完整页面契约和脚本见[展示规划](SHOWCASE_PLAN.md)。以下描述保留了当前已实现的本机控制台和 Pair 展屏设计，不代表目标页面均已上线。
