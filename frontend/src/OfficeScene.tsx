...
const roleNames:Record<string,string>={sentry:'监测员',monitor:'监测员',diagnoser:'诊断员',diagnose:'诊断员','fixer-a':'修复员 A','fixer-b':'修复员 B',fixer:'修复员',repair:'修复员',verifier:'验收员',verify:'验收员',single:'独立助手',cost:'成本专家'}
...
export default function OfficeScene({agents,events,generation,activeRunId,incidentStatus,muted,connected}:{agents:OfficeAgent[];events:OfficeEvent[];generation:string;activeRunId:string|null;incidentStatus:string|null;muted:boolean;connected:boolean}){
 ...
 return <section className="s9-office" aria-label="Section 9 实时办公室">
   <div className="s9-office__room" data-testid="office-room"><div className={'s9-office__background-fallback '+(missingBackground?'':'asset-fallback-hidden')} aria-label="办公室布局"><span>监测区</span><span>会诊区</span><span>执行与验收区</span><span>待命区</span></div><img className="s9-office__background" src="/vendor/star-office/office_bg.webp" alt="Star Office 像素办公室背景" onLoad={()=>setMissingBackground(false)} onError={()=>setMissingBackground(true)}/><div className="s9-office__label s9-office__label--left">监测区</div><div className="s9-office__label s9-office__label--center">会诊区</div><div className="s9-office__label s9-office__label--server">执行与验收区</div><div className="s9-office__label s9-office__label--rest">待命区</div><div className="s9-office__badge"><span>{connected?'数据已连接':'连接中断'}</span>{muted&&<span>通信已暂停</span>}</div><img className="s9-office__desk s9-office__desk--left" src="/vendor/star-office/desk-v3.webp" alt="监测工位"/><img className="s9-office__desk s9-office__desk--center" src="/vendor/star-office/desk-v3.webp" alt="会诊工位"/><img className="s9-office__sofa" src="/vendor/star-office/sofa-idle-v3.png" alt="值班沙发"/>
     <div className="s9-office__archive">处置经验<br/><span>事件复盘记录 · 只读</span></div>
