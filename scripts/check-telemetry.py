...
def request_json(url: str, auth: str) -> tuple[int, object]:
...
def database_events(path: pathlib.Path, since: str) -> tuple[list[dict], str | None]:
    if not path.exists():
        return [], "missing"
    events: list[dict] = []
    try:
...
def main() -> int:
    ...
    for row in rows if isinstance(rows, list) else []:
        ...
        if is_local and name not in {"probe", "infra-probe"}:
            langfuse.append({
    ...
    for label, db_path in db_paths.items():
        found, error = database_events(db_path, since.isoformat().replace("+00:00", "Z"))
    ...
    for observation in langfuse:
        ...
        for event in candidates:
            ...
            if lf_run and db_run and lf_run != db_run:
                continue
            both.append({
            ...
            break
    complete = bool(both) and all(item["metadata_complete"] for item in langfuse if item.get("trace_id_full") in db_by_trace)
    ...
    record = {
        ...
        "criteria": "same full trace_id in Langfuse metadata observation and telemetry.span_received in main/evaluation/attract DB; metadata_complete must be true",
    }
    OUT.mkdir(parents=True, exist_ok=True)
    path = OUT / f"telemetry-check-{now.strftime('%Y%m%dT%H%M%SZ')}.json"
    path.write_text(json.dumps(record, ensure_ascii=False, indent=2) + "\n")
    print(f"status={status} langfuse_spans={len(langfuse)} db_events={len(db_events)} both_path_matches={len(both)} http_status={http_status} record={path}")
    for match in both[:5]:
