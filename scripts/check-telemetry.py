...
def main(argv: list[str] | None = None) -> int:
    ...
    record = {
        ...
        "status": status,
        "matching_spans": [{k: v for k, v in item.items() if k != "trace_id_full"} for item in langfuse],
        "database_events": [{k: v for k, v in item.items() if k != "trace_id_full"} for item in db_events],
        "both_path_matches": both,
        "counts": {"langfuse": len(langfuse), "collector": len(db_events), "matched": len(both)},
