...
try{
 ...
 if(old){const page=pages.swarm;const pattern=`**/api/pairs/${pair.pair_id}/arms/swarm`;await page.route(pattern,async route=>{const response=await route.fetch();await new Promise(r=>setTimeout(r,800));await route.fulfill({response}).catch(()=>{})});await page.getByRole('button',{name:'刷新对照记录'}).click();await page.getByLabel('选择历史对照记录').selectOption(old.pair_id);await wait(async()=>await page.locator('.sc-page').getAttribute('data-run-id')===old.swarm_run_id,'switch history');await page.waitForTimeout(1500);assert.equal(new URL(page.url()).searchParams.get('pair_id'),old.pair_id);assert.equal(await page.locator('.sc-page').getAttribute('data-pair-id'),old.pair_id);assert.equal(await page.locator('.sc-page').getAttribute('data-run-id'),old.swarm_run_id);await page.unroute(pattern);check('late_old_scope_response_cannot_replace_selected_pair',old.pair_id);await page.getByLabel('选择历史对照记录').selectOption(pair.pair_id);await wait(async()=>await page.locator('.sc-page').getAttribute('data-run-id')===pair.swarm_run_id,'return current');}
