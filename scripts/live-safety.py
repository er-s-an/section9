...
async def case(name, partial):
    ...
    try:
        ...
        if background:
            await background
        record = {"case": name, "test_passed": True, "origin": "live_model_and_production_state_machine",
                  "plan_origin": "deliberate_test_input_not_agent_output", "run": store.run(run["id"]),
                  "detection": detected, "verification": closed, "events": store.events(run_id=run["id"], limit=5000),
        ...
        print(json.dumps({"case": name, "test_passed": True, "run_id": run["id"], "file": str(filename)}, ensure_ascii=False), flush=True)
    finally:
