...
最终范围包括事故发现与处置、代码审查、成本优化、基础设施、安全评估、自动化、知识与经验、团队治理等工作流。[完整功能对标规划](docs/planning/Section9_完整功能对标与蜂群架构实施规划.md)与[JSON 登记表](docs/planning/Section9_功能对标登记表.json)记录目标范围；[展示规划](docs/SHOWCASE_PLAN.md)说明它与展位主线的关系。功能存在、产品可交付、蜂群效果分别验收，不把规划写成现有能力。

**本次公开 `main` 的可运行范围**：本机 macOS 控制面、固定小智示例应用的真实模型故障实验、独立验收和同案 Pair 双屏。模型推理依赖远程 EvoMap API。本地权限、租约/fencing、事件和证据导出已有实现；官方 EvoMap 远端协作会话、Hub 发布/检索及多场景蜂群优势尚未验收。[交付记录](docs/DELIVERY.md)与[双屏交付记录](docs/SHOWCASE_DELIVERY.md)保留对应版本的证据，[当前演示指南](docs/DEMO.md)给出可复跑路径。后续本地开发分支的外部应用实验尚未随本次文档上传发布到 `main`。

**展示主线**：评委触发一件 AI 应用故障 → 查看成员交换线索、反证与接管 → 审查受控修复 → 打开独立业务验收 → 查看同案单 Agent 对照和原始运行证据。网页中的办公室只映射真实任务事件，不能代替证据。最终展示脚本、网页信息架构、提交物和实验口径见[展示规划](docs/SHOWCASE_PLAN.md)与[展示研究](docs/section9-demo-research-20260923.md)。
